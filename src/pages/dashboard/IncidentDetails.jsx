  ...
  return (
    <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }}
      ...
      <motion.div initial={{ scale: 0.95, opacity: 0 }} animate={{ scale: 1, opacity: 1 }} exit={{ scale: 0.95, opacity: 0 }}
        ...
        <p className="text-sm text-gray-500 mb-6">Confirm this incident has been handled. This will be logged in the timeline.</p>
        <div className="flex gap-3">
          ...
          <button onClick={onConfirm} className="flex-1 py-2.5 bg-green-600 text-white text-sm font-semibold rounded-xl hover:bg-green-700 transition-colors">Yes, Resolve</button>
        </div>
      </motion.div>
    </motion.div>
  )
